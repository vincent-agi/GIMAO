...
- Composable `useApi` (via `composables/http.js`, wrapper axios) — **déjà fonctionnel et utilisé dans 63 fichiers** (le fichier contient d'anciens brouillons commentés en tête de fichier, mais le code actif en fin de fichier est complet, avec gestion loading/error et JSDoc). Correction par rapport à une évaluation erronée de ce même bilan en première lecture partielle du fichier. Seules 2 vues faisaient encore un accès direct à axios/`http.js` (`SetPassword.vue` — import mort, déjà utilisait `useApi` en pratique — et `ExportData.vue` — export de fichier nécessitant les en-têtes de réponse, non couvert par l'API de `useApi` à l'époque) ; cf. TUS-007.
...
9. ~~`useApi.js` (frontend) largement commenté/inactif~~ — **correction (TUS-007) : déjà fonctionnel et utilisé dans 63 fichiers.** Seuls 2 écarts trouvés (import mort dans `SetPassword.vue`, accès direct nécessaire dans `ExportData.vue` faute de méthode exposant la réponse Axios complète) — corrigés en ajoutant `useApi.getRaw()` pour les cas nécessitant les en-têtes/blob.
