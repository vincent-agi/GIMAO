<template>
  ...
  >
    <template #default="{ data }">
      ...
      <v-row v-else>
        <v-col>
          <v-alert type="info" outlined> Aucune donnée disponible pour ce fabricant. </v-alert>
        </v-col>
      </v-row>
    </template>
  </BaseDetailView>
  ...
  <v-btn
    v-if="store.getters.hasPermission('man:edit')"
    color="primary"
    size="large"
    icon
  ...
  >
    ...
    <v-tooltip activator="parent" location="left"> Modifier le fabricant </v-tooltip>
  </v-btn>
</template>
