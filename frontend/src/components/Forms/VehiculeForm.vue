<template>
  <BaseForm
    ...
    @cancel="close"
  >
    <v-card-subtitle class="text-h6 font-weight-bold px-0 pb-2"> Identification </v-card-subtitle>
    ...
    <v-row dense>
      ...
      <v-col cols="12" md="4">
        <FormField
          v-model="formData.puissanceFiscale"
          field-name="puissanceFiscale"
...
<script setup>
  ...
  const validationSchema = {
    ...
    vin: [
      ...
      {
        name: 'pattern',
        params: [/^[A-HJ-NPR-Z0-9]{17}$/],
        message: 'Le VIN doit comporter 17 caractères (hors I, O, Q)',
      },
    ],
    immatriculation: [
      'required',
