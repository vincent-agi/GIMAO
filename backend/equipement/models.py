...
class ModeleEquipement(models.Model):
    ...
    fabricant = models.ForeignKey(
        Fabricant, on_delete=models.PROTECT, related_name="modeles", help_text="Fabricant du modèle"
    )
...
class FamilleEquipement(models.Model):
    ...
    familleParente = models.ForeignKey(
        "self",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
        related_name="sous_familles",
        help_text="Famille parente, si applicable",
    )
...
class Equipement(ArchivableMixin, models.Model):
    ...
    numSerie = models.CharField(
        max_length=100, null=True, blank=True, help_text="Numéro de série de l'équipement"
    )
    reference = models.CharField(
        max_length=100, null=True, blank=True, help_text="Référence interne ou fournisseur"
    )
    dateCreation = models.DateTimeField(
        auto_now_add=True, help_text="Date de création de l'équipement"
    )
    designation = models.CharField(max_length=100, help_text="Nom ou désignation de l'équipement")
    type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, null=True, blank=True, help_text="Type d'équipement"
    )
    dateMiseEnService = models.DateTimeField(
        null=True, blank=True, help_text="Date de mise en service de l'équipement"
    )
    prixAchat = models.DecimalField(
        ...
        null=True,
        blank=True,
        help_text="Prix d'achat de l'équipement",
    )
    lienImage = models.FileField(
        upload_to="equipement_images/", null=True, blank=True, help_text="Image de l'équipement"
    )
    ...
    lieu = models.ForeignKey(
        ...
        help_text="Lieu où se trouve l'équipement",
    )
    ...
    fournisseur = models.ForeignKey(
        ...
        blank=True,
        related_name="equipements",
        help_text="Fournisseur de l'équipement",
    )
    ...
    famille = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        related_name="equipements",
        help_text="Famille de l'équipement",
    )
    x = models.FloatField(
        null=True, blank=True, help_text="Coordonnée X de l'équipement dans le lieu"
    )
    y = models.FloatField(
        null=True, blank=True, help_text="Coordonnée Y del'équipement dans le lieu"
    )
...
class StatutEquipement(models.Model):
    ...
    statut = models.CharField(
        max_length=50, choices=STATUTS_CHOICES, help_text="Statut de l'équipement"
    )
    dateChangement = models.DateTimeField(
        auto_now_add=True, help_text="Date du changement de statut"
    )
    equipement = models.ForeignKey(
        Equipement,
        on_delete=models.CASCADE,
        related_name="statuts",
        help_text="Équipement concerné",
    )
...
class Compteur(models.Model):
    ...
    nomCompteur = models.CharField(
        max_length=100, null=False, default="Compteur sans nom", help_text="Nom du compteur"
    )
    valeurCourante = models.FloatField(help_text="Valeur actuelle du compteur")
    unite = models.CharField(
        max_length=50, help_text="Unité de mesure du compteur", default="jours"
    )
    estPrincipal = models.BooleanField(
        default=False, help_text="Indique si ce compteur est le principal pour l'équipement"
    )
    type = models.CharField(
        max_length=25, null=False, default="Général", help_text="Type du compteur"
    )
    ...
    equipement = models.ForeignKey(
        ...
        help_text="Équipement associé au compteur",
    )
...
class Declencher(models.Model):
    ...
    derniereIntervention = models.IntegerField(
        default=0, help_text="Valeur du compteur à la dernière intervention"
    )
    ...
    estGlissant = models.BooleanField(default=False, help_text="Indique si le seuil est glissant")
    ...
    compteur = models.ForeignKey(Compteur, on_delete=models.CASCADE, related_name="declenchements")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["compteur", "planMaintenance"], name="unique_declenchement_compteur_valeur"
            )
        ]
...
class Constituer(models.Model):
    ...
    equipement = models.ForeignKey(
        Equipement, on_delete=models.CASCADE, help_text="Équipement associé"
    )
    consommable = models.ForeignKey(
        Consommable, on_delete=models.CASCADE, help_text="Consommable associé"
    )
